...
export const VideoResourceCard = ({ 
  resource, 
  showBadge = true,
  translationNamespace = 'resources'
}: VideoResourceCardProps) => {
  const tResources = useTranslations(translationNamespace);
  ...
  }, [videoData.youTubeVideoId, isMobile, videoPlayer, translatedContent]);

  // Handle inline close with cleanup
  ...
  useEffect(() => {
    ...
    const createPlayerWithDelay = async (retryCount = 0) => {
      try {
        ...
        // Ensure no existing player with this ID
