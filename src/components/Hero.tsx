...
const DecorativeElement = ({ className, type = 'circle', animate = false }: DecorativeElementProps) => {
  const baseClasses = `absolute opacity-30 ${animate ? 'animate-pulse' : ''}`;
  const shapeClasses = type === 'circle' ? 'rounded-full' : 'rotate-45';
...
export const Hero = ({ 
  ...
  if (layout === 'magazine') {
    return (
      <section className={`relative overflow-hidden min-h-screen flex items-center justify-center ${backgroundClasses[backgroundVariant]} ${className}`}>
        {/* Background overlay */}
  ...
  return (
    <section className={`relative overflow-hidden min-h-[80vh] flex items-center ${backgroundClasses[backgroundVariant]} ${className}`}>
      {/* Background overlay */}
      ...
      <div className={`relative z-10 px-6 py-20 w-full ${contentMaxWidth}`}>
