...
export const Stats = ({ stats, columns = 4, className = "" }: StatsProps) => {
  ...
  return (
    <div className={clsx('grid gap-6 text-center', columnClasses[columns], className)}>
      {stats.map((stat, index) => (
        ...
        >
          <div className={clsx(
            'text-5xl md:text-6xl lg:text-7xl font-black mb-4 magazine-headline',
            colorClasses[stat.color || 'accent-primary'],
            stat.value === '∞' && 'infinity-symbol'
          )}>
            {stat.value}
          </div>
